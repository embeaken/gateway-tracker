...
**Layout:** `App.vue` uses `MainLayout` (two-column grid with `#content` and `#sidebar` slots). Content slot holds the `ProjectCard` list. Sidebar holds `ActivityTimeline`. A `ContextCard` modal (teleported to body) provides background info.

**Data:** All data is static — no runtime API calls. Projects come from `src/assets/data.ts`. Timeline items come from `src/assets/activityData.json` (generated by `scripts/update_data.py`), re-exported with types via `src/assets/activityData.ts`. Types are in `src/types.ts` and `src/assets/activityDataTypes.ts`. Some components (AlertNotice) have inline hardcoded content.
